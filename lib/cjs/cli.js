...
function parseArgs(argv) {
    const args = { pretty: false, help: false, version: false };
    for (let i = 0; i < argv.length; i++) {
        ...
        switch (arg) {
            ...
            case "--pretty":
                ...
                break;
            case "--latitude":
                args.latitude = argv[++i];
                break;
            case "--longitude":
                args.longitude = argv[++i];
                break;
            case "-o":
            case "--output":
                args.output = argv[++i];
...
function main() {
    ...
    const csv = (0, fs_1.readFileSync)(args.file ?? 0, "utf8");
    ...
    if (args.output) {
        (0, fs_1.writeFileSync)(args.output, json + "\n");
    }
    else {
